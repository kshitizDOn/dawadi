...
module.exports = {
  ...
  run: async function ({ api, event, args, config }) {
    try {
      ...
      const apiUrl = `https://code-merge-api-hazeyy01.replit.app/pinterest/api?search=${encodeURIComponent(
        keySearchs
      )}`;
      ...
      await this.cleanFolder(path.join(__dirname, "cache"));
  ...
  cleanFolder: async function (folderPath) {
    try {
